...
const InternalJobs: React.FC = () => {
  ...
  const fetchJobsData = async () => {
    try {
      const response = await sp.web.lists.getByTitle("Internal_recrutement").items.select("offer_title", "short_description", "deadline", "city").get();
  ...
  useEffect(() => {
    const getData = async () => {
      ...
      setFilteredJobs(InternalJobs); // Initialize filtered jobs with all jobs
